...
class MemoryManager:
    ...
    async def get_short_term(self, conversation_id: str) -> list[dict]:
        r = await self._r()
        raw = await r.get(f"mem:short:{conversation_id}")
    ...
    async def append_short_term(
        ...
        if len(history) > MAX_SHORT_TERM_TURNS:
            await self._flush_long_term(conversation_id, history, tenant_id)
            history = history[-5:]
        await r.setex(f"mem:short:{conversation_id}", settings.checkpoint_ttl, json.dumps(history))

    async def _flush_long_term(
        self, conversation_id: str, history: list[dict], tenant_id: str
    ) -> None:
        try:
            self._mem0.add(
                messages=history,
    ...
    async def build_memory_context(
        ...
        short = await self.get_short_term(conversation_id)
