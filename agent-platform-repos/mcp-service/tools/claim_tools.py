...
async def query_claim_status(claim_id: str) -> dict:
    ...
    try:
        ...
        return {
            "claim_id": data["claimId"], "status": data["status"],
            "current_step": data["currentStep"], "submit_date": data["submitDate"],
...
async def list_claims_by_policy(policy_id: str) -> dict:
    ...
    try:
        ...
        return {
            "total": data["total"],
            "claims": [{"claim_id": c["claimId"], "status": c["status"],
